...
class AudienceSignature(BaseModel):
    ...
    reactions: List[str] = Field(
        description="Short reaction from each audience member explaining their inner thought process when hearing the joke (one per profile, same order)"
    )
    responses: List[Literal["hilarious", "funny", "meh", "not funny", "offensive"]] = (
        Field(
            description="Rating from each audience member (one per profile, same order)"
        )
    )
...
class AudienceEvaluationResult(BaseModel):
    ...
    responses: List[Literal["hilarious", "funny", "meh", "not funny", "offensive"]] = (
        Field(description="Individual ratings")
    )
...
class JokeGeneratorState(TypedDict, total=False):
    ...
    audience_responses: List[
        Literal["hilarious", "funny", "meh", "not funny", "offensive"]
    ]
