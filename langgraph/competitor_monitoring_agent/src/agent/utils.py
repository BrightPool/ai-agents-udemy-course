...
from agent.models import ArticleSummary, CompetitorSource, DigestEmail, DiscoveredLink
...
class AnchorExtractor(HTMLParser):
    ...
    def handle_starttag(
        ...
        """Handle HTML start tags, extracting href attributes from anchor tags."""
        if tag.lower() != "a":
            return
...
@dataclass
class CrawledLinks:
    """Container for crawled and normalized competitor links."""
...
def _build_pygsheets_client(
    *,
    ...
    authorize_kwargs: Dict[str, Any] = {"retries": 3, "local": False}
    ...
    if service_account_env_var:
        return pygsheets.authorize(
            service_account_env_var=service_account_env_var, **authorize_kwargs
        )

    if service_account_file:
        return pygsheets.authorize(
            service_account_file=service_account_file, **authorize_kwargs
        )

    # For public sheets, try without authentication - this may fail
    # but let's try the simplest approach first
    try:
        return pygsheets.authorize(**authorize_kwargs)
    except Exception as exc:
        raise RuntimeError(
            f"Unable to authenticate with Google Sheets: {exc!s}. "
            "For public sheets, you may need to provide service account credentials."
        ) from exc


def _convert_sheet_url_to_csv(sheet_url: str) -> str:
    """Convert a Google Sheets URL to CSV export format for public sheets."""
    ...
    if "/d/" in sheet_url:
        ...
        raise ValueError(f"Unable to extract sheet ID from URL: {sheet_url}")
...
def fetch_competitor_records(
    ...
    *,
    ...
    """Fetch competitor rows from Google Sheets via CSV export for public sheets."""
    # For public sheets, try CSV export first (no auth required)
    try:
        csv_url = _convert_sheet_url_to_csv(sheet_url)
        csv_content = fetch_text(csv_url)
        ...
        # If CSV export fails, try pygsheets with authentication
        try:
            ...
            worksheet_obj = (
                spreadsheet.worksheet_by_title(worksheet)
                if worksheet
                else spreadsheet.sheet1
            )
            return worksheet_obj.get_all_records(empty_value="")

        except Exception as pygsheets_exc:  # noqa: BLE001
            raise RuntimeError(
                f"Unable to load Google Sheet at {sheet_url}. "
                f"CSV export failed: {csv_exc!s}. "
                f"Pygsheets failed: {pygsheets_exc!s}. "
                "For private sheets, provide service account credentials."
            ) from pygsheets_exc


def parse_competitor_records(
    ...
    for row in records:
        ...
        # Try multiple column name variations for company
        company = (
            normalized_row.get("company")
            or normalized_row.get("name")
            or normalized_row.get("competitor")
            or normalized_row.get("company name")
            or normalized_row.get("organization")
            or normalized_row.get("brand")
        )

        # If still no company name, try to infer from URL
        if not company or company.lower() == "unknown":
            if root_url:
                # Extract domain from URL and capitalize it
                ...
                inferred_company = domain.split(".")[0].capitalize()
                # Use the inferred company name
...
def normalize_links(
    competitor: CompetitorSource, raw_links: Iterable[str]
) -> CrawledLinks:
...
def write_email_to_outbox(
    email: DigestEmail, outbox_dir: Path = DEFAULT_OUTBOX_DIR
) -> Path:
