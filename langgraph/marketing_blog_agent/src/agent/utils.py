"""Utility functions and data for the marketing blog agent.

This module contains shared utilities, example data, and helper functions
that support the blog writing workflow.
"""
...
EXAMPLE_DOCS: List[dict] = [
    {
        "id": "company_vision_1",
        "text": "Nimbus is the revenue automation platform for RevOps and Data teams. We replace brittle spreadsheets with governed, AI-assisted workflows.",
    },
    {
        "id": "pricing_tiers_1",
    ...
    {
        "id": "integrations_streaming_1",
        "text": "Streaming: Kafka and Segment sources supported; exactly-once event delivery with idempotency keys.",
    },
    {
        "id": "personas_1",
        "text": "Personas: RevOps needs pipeline visibility; Data Engineering needs reliable ingestion; Marketing Ops needs attribution sanity.",
    },
    {
        "id": "brand_voice_1",
        "text": "Voice: practical, no-hype, crisp verbs, short sentences. Avoid exclamation marks.",
    },
    {
        "id": "style_guide_1",
        "text": "Style: use US English, Oxford comma, and sentence case for headings.",
    },
    {
        "id": "product_features_1",
        "text": "Features: Rules Engine, Playbooks, and Workflows. Rules Engine executes row-level policies with audit logs.",
    },
    {
        "id": "security_1",
    ...
    {
        "id": "programs_migration_1",
        "text": "Concierge Migration: free one-time program up to 20 hours; includes schema mapping and QA.",
    },
    {
        "id": "event_growth_summit_1",
    ...
    {
        "id": "limits_api_1",
        "text": "API limits: 600 requests/min per org and 10 requests/sec per user. 429 means back off.",
    },
    {
        "id": "data_retention_1",
        "text": "Data retention: logs stored 30 days by default; retention can be extended on Scale.",
    },
    {
        "id": "roadmap_1",
    ...
    {
        "id": "billing_1",
        "text": "Billing: Annual contracts only; invoices net-30; procurement often requests a security questionnaire.",
    },
]
...
def _embed_texts(texts: List[str]) -> np.ndarray:
    """Generate vector embeddings for text using OpenAI API or fallback method.

    This function creates numerical vector representations of text inputs for
    semantic similarity calculations. It uses OpenAI's text-embedding-3-large
    model when available, with a deterministic random fallback for offline
    or API-unavailable scenarios.

    Args:
        texts: List of text strings to embed

    Returns:
        numpy.ndarray: 2D array of embeddings with shape (n_texts, embedding_dim)

    The embeddings enable semantic search and similarity matching across the
    marketing knowledge corpus, supporting the blog writing research process.
    """
    try:
